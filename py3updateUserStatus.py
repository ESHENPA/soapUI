...
def updateSubscriber(MSISDN, UserStatus,soap_host):
    ...
    conn = http.client.HTTPConnection(soap_host, timeout=10)
...
if __name__ == "__main__":
    while True:
        ...
        soapPGdict1718x = {"1718xPG":{"QDAPCRF06PG01BER":"10.14.59.11:8080","QDAPCRF07PG01BER":"127.0.0.1:10007","QDAPCRF08PG01BER":"127.0.0.1:10008"}}
        ...
        print (soapPGdict13x)
        print(soapPGdict1718x)
        print(soapPGdict1519x)

        MSISDN = str(input("请输入需要更新的号码+86，Warning请先查询号码是否已经签约,输入exit退出程序:"))
        if MSISDN=="exit":
            print ("updateUserStatus程序退出,再见")
            break
        UserStatus = str(input("请输入UserStatus编码 1为正常速率，51为限速1M，52为限速128K:"))
        soap_host = str(input("请输入用户所在PG的IP和端口例如 1.1.1.1:8080 :"))
