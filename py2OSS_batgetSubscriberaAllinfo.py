...
class BatgetAllinfo():
    ...
    def successlist(self, MSISDN, city, soap_host, response):
        successlist = []
    ...
    def failedlist(self, MSISDN, city):
        failedlist = []
    ...
    def resultStore(self, csvWr, MSISDN, city, soapPGList):
        ...
        for soap_host in soapPGList:
            ...
            if "<resultCode>0</resultCode>" in response:
                replistsuccess = self.successlist(MSISDN, city, soap_host, response)
        ...
        if True not in responselist:
            respfailedlist = self.failedlist(MSISDN, city)
